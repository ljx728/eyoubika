...
/*==========================================================================================*
 * Description:	抓取各个交易所的申购挂牌信息
 * Class:		SubscribeSpiderAL
 * Author:		lijiaxuan
 * Copyright:	ABC (c) 2015 ljx728@gmail.com
 * History:		1.0 created by lijiaxuan at 2015年10月7日 上午10:11:22
 *==========================================================================================*/
public class SubscribeSpiderAL extends BaseSpider {
	...
	public void fetchSubscribeArticle(){	
		...
		exMap.put(CommonVariables.EXCHANGE_NANJING, "http://www.zgqbyp.com/web/news/108.html");//南京文交所
		exMap.put(CommonVariables.EXCHANGE_NANFANG, "http://www.nfqbyp.com/infomation.html?newsTypeID=17509&newsType=%E7%94%B3%E8%B4%AD%E6%8C%82%E7%89%8C"); //南方文交所
		exMap.put( CommonVariables.EXCHANGE_ZHONGNAN, "http://www.znypjy.com/a/xinxipilu/shengougonggao/");	//中南文交所
		exMap.put(CommonVariables.EXCHANGE_FULITE, "http://dzp.wjybk.com/?action-category-catid-127"); //福丽特
		exMap.put(CommonVariables.EXCHANGE_JINMAJIA, "http://qbyp.jinmajia.com/article/mtbd/qbyp/gggs/sggp/"); //金马甲
		exMap.put(CommonVariables.EXCHANGE_SHANGHAI, "http://www.shscce.com/html/shscce/zxgg/index_1.shtml");//上海
		exMap.put(CommonVariables.EXCHANGE_YIJIAO, "http://www.cacecybk.com.cn/web/news/news.jsp?parentid=44&classid=56");//艺交所	
		...
		while (index < fetchListSize) {
			...
			try	{		        	   	  
		        ...
		        threadPool.execute(new SubscribeFetcher( exId,  exMap.get(exId) , subscribeArticleDao)); //多线程抓取：每个交易所一个线程。      
 ...
 *==========================================================================================*/
class SubscribeFetcher extends BaseSpider implements Runnable{
	private  String exId;	//交易所
	private String url;		//目标网页入口URL
	private SubscribeArticleDao subscribeArticleDao;	//数据库层
	public SubscribeFetcher(String exId, String url, SubscribeArticleDao subscribeArticleDao){
		this.exId = exId;	
		this.url = url;
		this.subscribeArticleDao= subscribeArticleDao;
	}
	...
	private void nanjing001000Fetcher(){
		...
		String maxDate = subscribeArticleDao.findMaxDateByExId(exId);	//数据库最大文章网站时间
		int i = 0;
		int j = 0;
		...
		//for (i = 1 ; i <=  1 && fetchFlag; i++){
			//构造每一个申购信息列表页的URL
			if ( i == 1){
				pageUrl = "http://www.zgqbyp.com/web/news/108.html";
			} else {
				pageUrl = "http://www.zgqbyp.com/web/news/108_"+i+".html";
			}
			doc = fetchDocument(pageUrl);//获取网页源码
			
			pageEl = doc.select("div.ilist").first();//获取申购信息部分的源码
			...
			//for (j = 0; j < 1 && fetchFlag; j++){
				...
				articleUrl = this.buildUrl(articleUrl);//构造申购信息详情页的URL
	...
	/*--------------------------------------------------------------------------------------*
	 * Description:	南京文交所：解析申购信息详情页并保存
	...
	private void nanjing001000Saver(ArticleDomain articleDomain){
		...
		articleDomain.setType(CommonVariables.NOTE_SUBSCRIBE);
		subscribeArticleDao.addSubscribeArticle(articleDomain);		//保存到数据库
		System.out.println("articleDomain " + articleDomain.toString());
	}
	//===================================== 南方交易所 =====================================//
	/*--------------------------------------------------------------------------------------*
	 * Description:	南方交易所：获取申购信息总页数
	...
	private void nanfang002000Fetcher(){
		...
		//for (i = 1 ; i <= 1 && fetchFlag; i++){
			//构造每一个申购信息列表页的URL
			pageUrl = "http://www.nfqbyp.com/infomation.html?pageIndex=" + i + "&newsTypeID=17509&newsType=%E7%94%B3%E8%B4%AD%E6%8C%82%E7%89%8C";
			doc = fetchDocument(pageUrl);//获取网页源码
			pageEl = doc.select("div.cnews_content").first();//获取申购信息部分的源码
			articleEls = pageEl.select("li");
			articleElSize = articleEls.size();
			//解析获取每条申购信息
			for (j = 0; j < articleElSize && fetchFlag; j++){
	...
	/*--------------------------------------------------------------------------------------*
	 * Description:	南方交易所：解析申购信息详情页并保存
	...
	private void zhongnan003000Fetcher(){
		...
		//for (i = 1 ; i <= 1 && fetchFlag; i++){
			//构造每一个申购信息列表页的URL
			pageUrl = "http://www.znypjy.com/a/xinxipilu/shengougonggao/list_9_"+ i +".html";
			doc = fetchDocument(pageUrl);//获取网页源码
			
			pageEl = doc.select("div.main_right").first();//获取申购信息部分的源码
	...
	/*--------------------------------------------------------------------------------------*
	 * Description:	中南文交所：解析申购信息详情页并保存
	...
	private void fulite004000Fetcher(){
		...
		//for (i = 1 ; i <= 1 && fetchFlag; i++){
			//构造每一个申购信息列表页的URL
			pageUrl = "http://dzp.wjybk.com/?action-category-catid-127-page-" + i;
			doc = fetchDocument(pageUrl);//获取网页源码
			
			pageEl = doc.select("div#article").first();//获取申购信息部分的源码
			articleEls = pageEl.select("li");
			articleElSize = articleEls.size();
	...
	/*--------------------------------------------------------------------------------------*
	 * Description:	福丽特交易所：解析申购信息详情页并保存
	...
	private void jinmajia005000Fetcher(){
		...
		//for (i = 1 ; i <=  1 && fetchFlag; i++){
			//构造每一个申购信息列表页的URL
			if ( i == 1){
				pageUrl = "http://qbyp.jinmajia.com/article/mtbd/qbyp/gggs/sggp/index.shtml";
			} else {
				pageUrl = "http://qbyp.jinmajia.com/article/mtbd/qbyp/gggs/sggp/index.shtml?" + i;
			}
			doc = fetchDocument(pageUrl);//获取网页源码
			
			pageEl = doc.select("ul.article").first();//获取申购信息部分的源码
			articleEls = pageEl.select("li");
			articleElSize = articleEls.size();
	...
	/*--------------------------------------------------------------------------------------*
	 * Description:	金马甲文交所：解析申购信息详情页并保存
	...
	private void shanghai007000Fetcher(){
		...
		//for (i = 1 ; i <= 1 && fetchFlag; i++){
			...
			pageEl = doc.select("div.list").first();//获取申购信息部分的源码
			articleEls = pageEl.select("li");
			articleElSize = articleEls.size();
	...
	/*--------------------------------------------------------------------------------------*
	 * Description:	上海文交所：解析申购信息详情页并保存
	...
	private void yijiao010000Fetcher(){
		...
		//for (i = 1 ; i <= 1 && fetchFlag; i++){
			//构造每一个申购信息列表页的URL
			pageUrl = "http://www.cacecybk.com.cn/web/news/news.jsp?parentid=44&classid=56";
			doc = fetchDocument(pageUrl);//获取网页源码	
			pageEl = doc.select("div.st_rightbor").first().select("div.list").first();//获取申购信息部分的源码
			//获取网站时间
			String dateList = "";
	...
	public void run()
    {
		switch (this.exId){	
			case CommonVariables.EXCHANGE_NANJING: {	//南京文交所
				System.out.println("this.exId " + this.exId);
			...
			default: {		
				//System.out.println("this.exId " + this.exId);
				//commonFetcher( CommonUtil.getHost(url), url, exId, this.subscribeArticleDao);
				break;
			}				
		}
