...
/*==========================================================================================*
 * Description:	TODO
 * Class:		NoticeSpiderAL
 * Author:		lijiaxuan
 * Copyright:	ABC (c) 2015 ljx728@gmail.com
 * History:		1.0 created by lijiaxuan at 2015年10月9日 下午5:32:34
 *==========================================================================================*/
public class NoticeSpiderAL extends BaseSpider {
	...
	public void fetchNoticeArticle(){	
		...
		exMap.put(CommonVariables.EXCHANGE_NANJING, "http://www.zgqbyp.com/web/news/10.html");//南京文交所
		exMap.put(CommonVariables.EXCHANGE_NANFANG, "http://www.nfqbyp.com/infomation.html?newsTypeID=16793&newsType=%E4%B8%AD%E5%BF%83%E9%80%9A%E5%91%8A"); //南方文交所
		exMap.put( CommonVariables.EXCHANGE_ZHONGNAN, "http://www.znypjy.com/a/xinxipilu/tongzhigonggao/list_7_1.html");	//中南文交所
		exMap.put(CommonVariables.EXCHANGE_FULITE, "http://dzp.wjybk.com/?action-category-catid-2"); //福丽特 - 交易公告
		exMap.put(CommonVariables.EXCHANGE_JINMAJIA, "http://qbyp.jinmajia.com/article/mtbd/qbyp/gggs/pttz/"); //金马甲
		exMap.put(CommonVariables.EXCHANGE_SHANGHAI, "http://www.shscce.com/html/shscce/zxgg/index_1.shtml");//上海
		exMap.put(CommonVariables.EXCHANGE_YIJIAO, "http://www.cacecybk.com.cn/web/news/news.jsp?parentid=44&classid=59");//艺交所	
		...
		while (index < fetchListSize) {
			...
			try	{		        	   	  
		        ...
		        threadPool.execute(new NoticeFetcher( exId,  exMap.get(exId) , noticeArticleDao)); //多线程抓取：每个交易所一个线程。      
 ...
 *==========================================================================================*/
class NoticeFetcher extends BaseSpider implements Runnable{
	private  String exId;	//交易所
	private String url;		//目标网页入口URL
	private NoticeArticleDao noticeArticleDao;	//数据库层
	public NoticeFetcher(String exId, String url, NoticeArticleDao noticeArticleDao){
		this.exId = exId;	
		this.url = url;
		this.noticeArticleDao= noticeArticleDao;
	}
	...
	private void nanjing001000Fetcher(){
		...
		String maxDate = noticeArticleDao.findMaxDateByExId(exId);	//数据库最大文章网站时间
		int i = 0;
		int j = 0;
		...
		//for (i = 1 ; i <=  1 && fetchFlag; i++){
			//构造每一个托管信息列表页的URL
			if ( i == 1){
				pageUrl = "http://www.zgqbyp.com/web/news/10.html";
			} else {
				pageUrl = "http://www.zgqbyp.com/web/news/10_"+i+".html";
			}
	...
	private void nanjing001000Saver(ArticleDomain articleDomain){
		...
		articleDomain.setType(CommonVariables.NOTE_NOTICE);
		noticeArticleDao.addNoticeArticle(articleDomain);		//保存到数据库
		//System.out.println("articleDomain " + articleDomain.toString());
	}
	//===================================== 南方交易所 =====================================//
	/*--------------------------------------------------------------------------------------*
	...
	private void nanfang002000Fetcher(){
		...
		//for (i = 1 ; i <= 1 && fetchFlag; i++){
			//构造每一个托管信息列表页的URL
			pageUrl = "http://www.nfqbyp.com/infomation.html?pageIndex="+ i +"&newsTypeID=16793&newsType=%E4%B8%AD%E5%BF%83%E9%80%9A%E5%91%8A";
			doc = fetchDocument(pageUrl);//获取网页源码
	...
	private void zhongnan003000Fetcher(){
		...
		//for (i = 1 ; i <= 1 && fetchFlag; i++){
			//构造每一个托管信息列表页的URL
			pageUrl = "http://www.znypjy.com/a/xinxipilu/tongzhigonggao/list_7_"+ i +".html";
			doc = fetchDocument(pageUrl);//获取网页源码
	...
	private void fulite004000Fetcher(){
		...
		//for (i = 1 ; i <= 1 && fetchFlag; i++){
			//构造每一个托管信息列表页的URL
			pageUrl = "http://dzp.wjybk.com/?action-category-catid-2-page-" + i;
			doc = fetchDocument(pageUrl);//获取网页源码
	...
	private void jinmajia005000Fetcher(){
		...
		//for (i = 1 ; i <=  1 && fetchFlag; i++){
			//构造每一个托管信息列表页的URL
			if ( i == 1){
				pageUrl = "http://qbyp.jinmajia.com/article/mtbd/qbyp/gggs/pttz/index.shtml";
			} else {
				pageUrl = "http://qbyp.jinmajia.com/article/mtbd/qbyp/gggs/pttz/index.shtml?" + i;
			}
			doc = fetchDocument(pageUrl);//获取网页源码
	...
	private void shanghai007000Fetcher(){
		...
		//for (i = 1 ; i <= 1 && fetchFlag; i++){
			...
			//for (j = 0; j < 5 && fetchFlag ; j++){
				...
				if (title.contains("入库") || title.contains("申购")){
					continue;
				}
				webTime = articleEls.get(j).select("span").text();
				articleUrl = this.buildUrl(articleUrl);//构造托管信息详情页的URL
	...
	private void yijiao010000Fetcher(){
		...
		//for (i = 1 ; i <= 1 && fetchFlag; i++){
			//构造每一个托管信息列表页的URL
			pageUrl = "http://www.cacecybk.com.cn/web/news/news.jsp?parentid=44&classid=59&pageIndex=" + i;
			doc = fetchDocument(pageUrl);//获取网页源码	
	...
	public void run()
    {
		switch (this.exId){	
			...
			default: {		
				...
				//commonFetcher( CommonUtil.getHost(url), url, exId, this.noticeArticleDao);
				break;
			}				
		}
