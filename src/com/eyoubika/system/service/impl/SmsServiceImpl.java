...
public class SmsServiceImpl extends BaseService implements ISmsService{
	...
	public Map<String, Object> getAuthcode(SmsVO smsVo){
		...
		//String authcode = ContentUtil.generateAuthcode();
		String authcode = "eyoubika";
		HttpSession session = request.getSession();	
		session.setAttribute(CommonVariables.SESSION_AUTHCODE, authcode);
		//session.setMaxInactiveInterval(Integer.parseInt(CommonVariables.SESSION_EXPIRETIME));
		return this.buildRetData( "0", smsVo.getToken());	//用户注册前，userId=0
	}
	/*--------------------------------------------------------------------------------------*
